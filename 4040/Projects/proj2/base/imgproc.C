...
/* I thought we would be implementing flip/flop in this project but apparently not.
 Leaving this in here though to work on later...
void ImgProc::flip()
{
   for(int j=0;j<Ny/2;j++){
     ...
     for(int i=0;i<Nx;i++){
       ...
       vector<float> C2;
       image.value(i,j,C);
       image.value(i,Ny-j-1,C2);
       image.set_value(i,Ny-j-1,C);
       image.set_value(i,j,C2);
     }
   }
}
...
void ImgProc::flop()
{
   for(int j=0;j<Ny;j++){
     for(int i=0;i<Nx/2;i++){
       ...
       vector<float> C2;
       image.value(i,j,C);
       image.value(Nx-i-1,j,C2);
       image.set_value(Nx-i-1,j,C);
       image.set_value(i,j,C2);
     }
   }
}

...
void FFTImgProc::doIFS(FFTImgProc& image){
...
void FFTImgProc::FractalFlameIFS(const size_t iterations, const ColorLUT& lut, FFTImgProc& out){
  Point P;
  srand48(time(0));
  P.x = 2.0*drand48() - 1.0;
  P.y = 2.0*drand48() - 1.0;
...
float FFTImgProc::fractalPolar(Point& P){
  //(x,y) = (0/π, r −1);
  float x = P.x, y = P.y;
...
float FFTImgProc::fractalHandkerchief(Point& P){
...
float FFTImgProc::fractalHeart(Point& P){
  //(x,y) = r · (sin(theta * r), − cos(theta * r))
  float x = P.x, y = P.y;
...
float FFTImgProc::fractalDisc(Point& P){
...
float FFTImgProc::Iminimum(int x){
...
float FFTImgProc::Imaximum(int x){
...
float FFTImgProc::Iaverage(int x){
  float sum = 0;
  //#pragma omp parallel for
     for( long i=x;i<(Nx*Ny);i+=Nc ){
       sum += img_data[i];
...
float FFTImgProc::IstdDev(int x){
  float stdDev = 0;

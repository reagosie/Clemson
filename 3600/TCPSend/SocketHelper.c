...
*
* outputs:
*      returns  EXIT_FAILURE or the number of bytes received 
*      
...
//                        struct sockaddr *src_addr, socklen_t *addrlen);
 ...
 if (rc != EXIT_SUCCESS)
 {
   ...
   printf("RxMsg: rc:%d and  msgSize:%d   \n",rc, msgSize);
 }
#endif
...
*
* Explanation:  This sends data over the socket 
*
* inputs:   
*   int sock : socket descriptor
...
*
* TODO: Change so returns EXIT_FAILURE or number bytes sent
*      
***************************************************/
...
*
* inputs:   
*      int sock :  a valid socket descriptor
*      int option :   a valid socket option (man 7 socket)
*      void *optionData  :  ptr to caller memory for additional data needed 
*      int sizeData :  size of caller's buffer
*
...
*
* inputs:   
*      int sock : valid socket descriptor
*      int option : valid value of the desired option
*
...
struct addrinfo *addr;
  ...
  for (addr = addrList; addr != NULL; addr = addr->ai_next) 
  {
    ...
    if (servSock < 0) {
#ifdef TRACEME
      perror("SetupTCPServer: the address failed, try the next in the list : ");
#endif
      continue;       // Socket creation failed; try next address
    }
    ...
    rc = NOERROR;
#ifdef TRACEME
     printf("SetupTCPServer: Selected an Address :  ");
  ...
  //Double check
  if (servSock < 0) {
    printf("SetupTCPServer: ERROR, ended loop with no socket ??");
    rc = ERROR;
  }
  ...
  if (rc == NOERROR)
  {
    ...
    //TODO: Use SetSocketOption()
    int enable = 1;
    rc = setsockopt(servSock, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
    if (rc == ERROR) {
      perror("SetupTCPServer: failed the setsockopt SO_REUSEADDR, just ignore the error : ");
    }

    // Bind to the local address and set socket to listen
    rc = bind(servSock, selectedAddr->ai_addr, selectedAddr->ai_addrlen);
    if ( rc == ERROR) {
      perror("SetupTCPServer: Failed the bind : ");
    } else {

#ifdef TRACEME
       printf("SetupTCPServer: Bind succeeded ...\n");
#endif

      rc =  listen(servSock, MAXPENDING);
      if ( rc == ERROR) {
        perror("SetupTCPServer: Failed listen \n");
      }
#ifdef TRACEME
      else
         printf("SetupTCPServer: Listen succeeded ...\n");
#endif
    }
    ...
    if (rc == NOERROR) {
      ...
      rc = getsockname(servSock, (struct sockaddr *) &localAddr, &addrSize);
      if ( rc == ERROR) {
        perror("SetupTCPServer: Failed getsockname ?? \n");
      }

#ifdef TRACEME
       printf("SetupTCPServer:  Selected addr:  (out of %d choices) \n",numberAddrChoices);
       PrintSocketAddress((struct sockaddr *) selectedAddr, stdout);
      ...
      fputc('\n', stdout);
       ...
       printf("SetupTCPServer: HARD ERROR 1: had an addr but FAILED ?? \n");
    }
  } else {
     printf("SetupTCPServer: HARD ERROR 2: had an addr but FAILED ?? \n");
  }

  // Free address list allocated by getaddrinfo()
  freeaddrinfo(addrList);

  if ((servSock == ERROR) || (rc == ERROR)) {
    printf("SetupTCPServer: HARD ERROR 3: returning an error \n");
    servSock = ERROR;
  } 

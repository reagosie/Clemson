...
int printSession(double curTime, FILE *fileFID, session *toprint) 
{
  ...
  if (durSecs > 0)
    ...
    printf("printSession(%d): session:client IP %s: , durSecs:%f  \n",
      ...
      lossEventRate=0.0;
      if (tmpVar > 0.0) 
        lossEventRate = toprint->lossEventCount/tmpVar;
      ...
      if (toprint->lossEventCount>0)
        avgMBL= toprint->messagesLost/(double)toprint->lossEventCount;
  ...
  fprintf(fileFID,"%12.9f %s:%d %4.9f %f %f %f %f  %12.0f bps %2.4f %2.4f %2.2f %2.9f %2.9f %2.9f \n", 
          curTime, cAddr, toprint->clientPort, 
          durSecs,
          toprint->messagesReceived, toprint->messagesLost, toprint->lossEventCount,

...
/* tfs_init()
 * -initializes the directory as empty and the file allocation table to have all blocks free
 * -no parameters
 * -no return value
 */
void tfs_init(){
...
/* tfs_list_blocks()
 * -list file blocks that are being used and next block values from the file allocation table
 * -no parameters
 * -no return value
 */
...
/* tfs_exists()
 * -return TRUE if a file name is associated with an active directory entry
 * -preconditions:
 *   (1) the name is valid
...
/* tfs_create()
 * -create a new directory entry with the given file name and set the status to open, the first_block to invalid, the
      size to 0 and the byte offset to 0
 * -preconditions:
 *   (1) the name is valid
 *   (2) the name is not already associated with any active directory entry
 ...
 * -input parameter is file name
 * -return value is the file descriptor of a directory entry when successful or 0 when failure
 */
...
/* tfs_open()
 * -opens the directory entry having the given file name and sets the status to open and the byte offset to 0
 * -preconditions:
 *   (1) the name is valid
 *   (2) the name is associated with an active directory entry
...
/* tfs_close()
 * -closes the directory entry having the given file descriptor (sets the status to closed and the byte offset to 0); the
 *   function fails if (1) the file descriptor is out of range, (2) the file descriptor is within range but the directory
 *   entry is not open
 * -preconditions:
 ...
 *   there are no changes to the file data structures
 * -input parameter is a file descriptor
 * -return value is the file size when successful or MAX_FILE_SIZE+1 when failure
 */
 ...
 * -postconditions:
 *   the byte offset of the directory entry is set to the specified offset
